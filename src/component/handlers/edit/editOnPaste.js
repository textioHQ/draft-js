...
function editOnPaste(e: SyntheticClipboardEvent): void {
  ...
  function handleText(text: string, html?: string) {
    ...
    if (
      _this.props.handlePastedText &&
      isEventHandled(_this.props.handlePastedText(text, html))
    ) {
      return;
    }
    ...
    if (!_this.props.stripPastedStyles) {
      ...
      if (data.isRichText() && internalClipboard) {
        if (
          ...
          this.update(
            insertFragment(_this.props.editorState, internalClipboard)
          );
          return;
        }
      } else if (
      ...
      if (html) {
        ...
        if (htmlFragment) {
          ...
          if (contentBlocks) {
            ...
            this.update(
              insertFragment(_this.props.editorState, htmlMap, entityMap)
            );
            return;
          }
        }
    ...
    if (textBlocks.length) {
      ...
      this.update(insertFragment(_this.props.editorState, textMap));
    }
  }
}
